...
OPS = [
    ...
    },
    {
        "op_name": "log",
        "family": "elementwise",
        "source_op": "linalg.generic(math.log)",
        "target_op": "rngd.elementwise",
        "dtype": "f32",
        "tolerance": "max(2%, 1e-4)",
        "experimental": 1,
        "status": "done",
        "compares": "PyTorch torch.log(x.abs().clamp(min=1e-6)) — [A=2048] f32",
        "meaning_pass": "FpUnaryOp::Log 사용. 입력은 양수(abs+clamp) 필수. PASS",
        "meaning_fail": "NaN 발생 시 입력에 음수 포함 — abs().clamp(min=1e-6) 적용 필요.",
        "detail": "log(x): x > 0 필수. 음수 입력 시 NaN. e2e_pipeline.py에서 clamp 패턴 감지 필요.",
    },
    {
        "op_name": "gelu",
        "family": "elementwise",
        "source_op": "linalg.generic(pow+mul+add+tanh+mul)",
        "target_op": "rngd.elementwise",
        "dtype": "f32",
        "tolerance": "max(5%, 1e-3)",
        "experimental": 1,
        "status": "done",
        "compares": "PyTorch F.gelu(x) — [A=2048] f32",
        "meaning_pass": "3패스: x³*0.044715 → (x+x³c)*√(2/π)→tanh → (1+tanh)*0.5*x. PASS",
        "meaning_fail": "오차 초과 시 패스별 누적 오차 확인. 5% 허용.",
        "detail": "GELU 3패스. MulF ALU 3개 제한(Mul0/Mul1/Fma)으로 단일 파이프라인 불가. 누적 f32 오차 최대 3%.",
    },
    {
        "op_name": "silu",
        "family": "elementwise",
        "source_op": "linalg.generic(sigmoid+mul)",
        "target_op": "rngd.elementwise",
        "dtype": "f32",
        "tolerance": "max(2%, 1e-4)",
        "experimental": 1,
        "status": "done",
        "compares": "PyTorch F.silu(x) — [A=2048] f32",
        "meaning_pass": "stash에 x 저장 → FpUnaryOp::Sigmoid → MulF(Stash). PASS",
        "meaning_fail": "오차 초과 시 sigmoid 정밀도 확인 필요.",
        "detail": "SiLU(x) = x * sigmoid(x). stash × mainstream 패턴으로 구현.",
    },
    {
        "op_name": "transpose",
        "family": "transpose",
...
EVIDENCE = [
    ...
    ("gelu",         "static",  "FpUnaryOp::Tanh + FpBinaryOp::MulF 조합 확인",                "furiosa-opt-std-0.3.0/src/engine/vector/op/mod.rs"),
    ("gelu",         "runtime", "GELU 3패스 정합성 검증 통과 (A=2048, tol=5%)",               "cargo furiosa-opt test: pilot_e2e_gelu — PASS"),
    ("silu",         "static",  "FpUnaryOp::Sigmoid + FpBinaryOp::MulF(Stash) 조합 확인",       "furiosa-opt-std-0.3.0/src/engine/vector/op/mod.rs"),
    ("silu",         "runtime", "SiLU(x)=x*sigmoid(x) 정합성 검증 통과 (A=2048 f32)",            "cargo furiosa-opt test: pilot_e2e_silu — PASS"),
